...
export default function Auth() {
  ...
  async function handleResetPassword() {
    ...
    try {
      ...
      if (error) throw error
      alert("Abbiamo inviato un link per reimpostare la password alla tua email!")
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-950 p-6 text-white">
      <div className="w-full max-w-md space-y-8 rounded-2xl bg-zinc-900 p-8 shadow-xl">
        <div className="text-center">
          ...
          <p className="mt-2 text-zinc-400">
            {isResetting
              ? 'Recupero password'
              : isSignUp
                ? 'Crea il tuo account collezionista'
                : 'Bentornato nella tua collezione'}
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <button
            onClick={isResetting ? handleResetPassword : handleAuth}
            disabled={loading}
          ...
          >
            {loading ? (
              ...
              <><KeyRound size={20} /> Invia link di ripristino</>
            ) : isSignUp ? (
